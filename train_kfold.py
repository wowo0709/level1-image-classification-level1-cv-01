...
def train(data_dir, model_dir, args):
    ...
    #scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=1, eta_min=0.0001, last_epoch=-1)
    #scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.00001, 
	#											max_lr=args.lr,step_size_up=args.lr_decay_step,mode="exp_range",gamma=0.7,
    ...
    kfold_list = dataset.split_dataset()

    kfold_loss, kfold_acc = [], []
    for index, fold in enumerate(kfold_list):
        print(f"Starting {index+1} fold.......")
        ...
        for epoch in range(args.epochs):
            ...
            with torch.no_grad():
                ...
                if f1 > best_f1_score: #val_acc > best_val_acc and val_loss < best_val_loss:
                    ...
                    torch.save(model.module.state_dict(), f"{save_dir}{index+1}/best.pth")
                    best_f1_score = f1
                torch.save(model.module.state_dict(), f"{save_dir}{index+1}/last.pth")
                print(
        ...
        kfold_acc.append(best_val_acc)
        kfold_loss.append(best_val_loss)


    print(f"{idx}KFold mean acc :", np.mean(kfold_acc),f", {idx}KFold mean loss :",np.mean(kfold_loss))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='kfold/exp', help='model save at {SM_MODEL_DIR}/{name}')
